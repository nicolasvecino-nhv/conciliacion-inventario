...
if file_fusion and file_infolog:
    ...
    # 5. AGRUPACIONES PARA EL CUADRO DE DIFERENCIAS (Aquí agrupamos sin pallet para la comparación general)
...
if comparativa is not None:
    ...
    with tab2:
        ...
        if not df_fallas_mostrar.empty:
            st.error(f"⚠️ Se detectaron {len(df_fallas_mostrar)} pallets con novedades o fallas de vencimiento.")
            ...
            with pd.ExcelWriter(output_fechas, engine='xlsxwriter') as writer:
                df_fallas_mostrar.to_excel(writer, index=False, sheet_name='Control_Vencimientos')
